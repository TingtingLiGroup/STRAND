...
def area(sdata: SpatialData, shape_key: str, recompute: bool = False) -> None:
    ...
    """
    if get_shape is None or set_shape_metadata is None:
        raise ImportError("This function requires Bento SpatialData pipeline utils (get_shape/set_shape_metadata).")
...
def span(sdata: SpatialData, shape_key: str, recompute: bool = False) -> None:
    ...
    def get_span(poly):
        if not poly:
            return np.nan
        shape_coo = np.array(poly.exterior.coords.xy).T
...
# ---------- PURE POLYGON HELPERS (no SpatialData) ----------

def polygon_area(poly):
    return np.nan if (poly is None or not poly) else poly.area

def polygon_bounds(poly):
    if poly is None or not poly:
        return dict(minx=np.nan, miny=np.nan, maxx=np.nan, maxy=np.nan)
    minx, miny, maxx, maxy = poly.bounds
    return dict(minx=minx, miny=miny, maxx=maxx, maxy=maxy)

def polygon_radius(poly):
    # use Bento original implementation
    return _shape_radius(poly)

def polygon_span(poly):
    # use Bento original span logic
    if poly is None or not poly:
        return np.nan
    shape_coo = np.array(poly.exterior.coords.xy).T if hasattr(poly, "exterior") else np.array(poly.coords.xy).T
    return int(distance_matrix(shape_coo, shape_coo).max())

def polygon_raster(poly, step=1):
    # use Bento original raster logic
    return _raster_polygon(poly, step=step)

def compute_cell_prereqs(cell_polygon, step=1):
    """
    生成 point_features 需要的 cell-level 字段（与 Bento 原逻辑一致）
    返回 dict，可直接塞进 df 的列：
      radius, raster, span, minx/miny/maxx/maxy, area
    """
    b = polygon_bounds(cell_polygon)
    return {
        "radius": polygon_radius(cell_polygon),
        "raster": polygon_raster(cell_polygon, step=step),
        "span": polygon_span(cell_polygon),
        "area": polygon_area(cell_polygon),
        **b,
    }