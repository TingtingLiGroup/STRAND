...
# --- optional heavy deps (do NOT block importing feature classes) ---
try:
    import dask
...
def analyze_points(
    ...
    if SpatialData is None or dask is None or TqdmCallback is None:
        raise ImportError(
            "analyze_points requires optional deps: spatialdata, dask, tqdm-dask. "
            "They are not available in this environment."
        )
    if tl is None or get_points is None:
        raise ImportError(
            "analyze_points requires Bento pipeline modules (tools, _utils). "
            "Please migrate them or use your toolbox pipeline instead."
        )
...
class ShapeProximity(PointFeature):
    ...
    def extract(self, df):
        ...
        # 确保 df["geometry"] 是 GeoSeries 类型
        if not isinstance(df["geometry"], gpd.GeoSeries):
            df["geometry"] = gpd.GeoSeries(df["geometry"])  # 转为 GeoSeries
        points_geo = df["geometry"]
...
class RipleyStats(PointFeature):
    ...
    def extract(self, df):
        ...
        if not isinstance(df["geometry"], gpd.GeoSeries):
            df["geometry"] = gpd.GeoSeries(df["geometry"])  # 转为 GeoSeries
        # 获取点
        # Get points as (n,2) numpy array for astropy
        geo = df["geometry"]
        # 如果是 GeoDataFrame，geo 本身就是 GeoSeries；如果不是，也能用 gpd.GeoSeries 包一层
        if not hasattr(geo, "x"):
             geo = gpd.GeoSeries(geo)

        points_xy = np.column_stack([geo.x.to_numpy(), geo.y.to_numpy()])
        # 如果点数太少，Ripley 没意义（避免后面再报错）
        if points_xy.shape[0] < 2 or radii.size == 0:
            return {
                "l_max": np.nan,
        ...
        stats = estimator.Hfunction(data=points_xy, radii=radii, mode="none")
        ...
        l_half_radius = estimator.Hfunction(
            data=points_xy, radii=[quarter_span], mode="none"
        )[0]
...
class ShapeEnrichment(PointFeature):
    ...
    def extract(self, df):
        ...
        if not isinstance(df["geometry"], gpd.GeoSeries):
            df["geometry"] = gpd.GeoSeries(df["geometry"])  # 转为 GeoSeries
        points_geo = df["geometry"]  # 保持 GeoSeries 类型，继续空间操作
...
def compute_rnaforest13_features(df: pd.DataFrame, instance_key: str, nucleus_key: str) -> Dict[str, float]:
    """
    用 Bento 原 feature classes 计算官方 13 个特征。
    输入 df 是“单个 cell-gene 的点表”，必须至少包含：
      - x, y
      - geometry (shapely Point)
      - {instance_key} (cell polygon)
      - {instance_key}_radius, {instance_key}_raster, {instance_key}_span, {instance_key}_minx/miny/maxx/maxy, {instance_key}_area
      - {nucleus_key} (nucleus polygon，可为空)
      - {nucleus_key}_index (核内点标记: "" 或非空)
    返回 dict：13 个 key -> value
    """
    out = {}

    # Distance (3): cell inner; nucleus inner + outer
    out.update(ShapeProximity(instance_key, instance_key).extract(df))
    if df[nucleus_key].values[0] is not None:
        out.update(ShapeProximity(instance_key, nucleus_key).extract(df))
    else:
        out[f"{nucleus_key}_inner_proximity"] = np.nan
        out[f"{nucleus_key}_outer_proximity"] = np.nan

    # Symmetry (3): cell inner; nucleus inner + outer
    out.update(ShapeAsymmetry(instance_key, instance_key).extract(df))
    if df[nucleus_key].values[0] is not None:
        out.update(ShapeAsymmetry(instance_key, nucleus_key).extract(df))
    else:
        out[f"{nucleus_key}_inner_asymmetry"] = np.nan
        out[f"{nucleus_key}_outer_asymmetry"] = np.nan

    # Dispersion (2): point + nucleus
    out.update(PointDispersionNorm(instance_key, instance_key).extract(df))
    if df[nucleus_key].values[0] is not None:
        out.update(ShapeDispersionNorm(instance_key, nucleus_key).extract(df))
    else:
        out[f"{nucleus_key}_dispersion_norm"] = np.nan

    # Density (5): ripley
    out.update(RipleyStats(instance_key).extract(df))

    # 只保留官方 13 个（避免带出 cell outer_*）
    keep = [
    ...
    return {k: out.get(k, np.nan) for k in keep}