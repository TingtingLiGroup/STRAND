...
class ConditionalGlobalColocalization():
    '''
        ...
        This implementation keeps the original InSTAnT statistical definitions,
        but avoids materializing the large num_cells x num_genes x num_genes
        probability tensor used by the original code.
    '''
    def __init__(self, all_pvals, transcript_count, alpha_cellwise=0.01, min_transcript=0,
                 show_det_pairs=0, high_precision=False, threads=1, precision_mode='high'):
        ...
        self._prepare_global_statistics(all_pvals)
        print("Global Colocalization initialized ..")

    def _prepare_global_statistics(self, all_pvals):
        """Prepare exact CPB summary statistics with a smaller memory footprint."""
        edge_bool = np.asarray(all_pvals < self.alpha, dtype=np.bool_)
        self.obs = edge_bool.sum(axis=0).astype(np.int64)
        self.num_cells = edge_bool.shape[0]
        self.num_genes = edge_bool.shape[1]
        print('Number of cells: %d, Number of genes: %d' % (self.num_cells, self.num_genes))

        global_genecount = edge_bool.sum(axis=(0, 1)).astype(np.float64).reshape([-1, 1])
        self.gene_pair_weight = np.matmul(global_genecount, global_genecount.T)

        tri = np.triu_indices(self.num_genes)
        self.n_edges_per_cell = np.empty(self.num_cells, dtype=np.float64)
        for c in range(self.num_cells):
            self.n_edges_per_cell[c] = edge_bool[c][tri].sum()
        del edge_bool

        self.denom_per_cell = np.empty(self.num_cells, dtype=np.float64)
        weight = self.gene_pair_weight
        for c in range(self.num_cells):
            curr_weight_pair = weight.copy()
            curr_weight_pair[self.transcript_count[c] <= self.min_transcript] = 0
            self.denom_per_cell[c] = curr_weight_pair[tri].sum() + self.p_mode

    def _prob_pair_for_cell(self, cell_id, i, j):
        w = self.gene_pair_weight[i, j] / self.denom_per_cell[cell_id]
        return 1 - np.power((1 - w), self.n_edges_per_cell[cell_id])
    ...
    def _compute_pval_serial(self, args):
        ...
        for cell_id in range(self.num_cells):
            if self.transcript_count[cell_id, i] > self.min_transcript and self.transcript_count[cell_id, j] > self.min_transcript:
                p_ij.append(self._prob_pair_for_cell(cell_id, i, j))
        pb = PoiBin(p_ij)
        pval_curr = pb.pval(self.obs[i, j].astype(int))
        if self.high_precision and pval_curr < 1e-13:
            pb_highres = PoissonBinomial(p_ij)
            pval_curr = pb_highres.pval(self.obs[i, j].astype(int))
        return i, j, pval_curr, np.sum(p_ij)

    def _initializer_func(self, transcript_count, gene_pair_weight, denom_per_cell, n_edges_per_cell,
                          obs_matrix, min_transcript, high_precision, num_cells):
        global _cg_transcript_count, _cg_gene_pair_weight, _cg_denom_per_cell
        global _cg_n_edges_per_cell, _cg_obs, _cg_min_transcript, _cg_high_precision, _cg_num_cells
        _cg_transcript_count = transcript_count
        _cg_gene_pair_weight = gene_pair_weight
        _cg_denom_per_cell = denom_per_cell
        _cg_n_edges_per_cell = n_edges_per_cell
        _cg_obs = obs_matrix
        _cg_min_transcript = min_transcript
        _cg_high_precision = high_precision
        _cg_num_cells = num_cells

    @staticmethod
    def _compute_pval_parallel(args):
        i, j = args[0], args[1]
        p_ij = []
        for cell_id in range(_cg_num_cells):
            if _cg_transcript_count[cell_id, i] > _cg_min_transcript and _cg_transcript_count[cell_id, j] > _cg_min_transcript:
                w = _cg_gene_pair_weight[i, j] / _cg_denom_per_cell[cell_id]
                p_ij.append(1 - np.power((1 - w), _cg_n_edges_per_cell[cell_id]))
        pb = PoiBin(p_ij)
        pval_curr = pb.pval(_cg_obs[i, j].astype(int))
        if _cg_high_precision and pval_curr < 1e-13:
            pb_highres = PoissonBinomial(p_ij)
            pval_curr = pb_highres.pval(_cg_obs[i, j].astype(int))
        return i, j, pval_curr, np.sum(p_ij)

    def global_colocalization(self):
        start = timeit.default_timer()
        # tasks = [[i, j] for i in range(self.num_genes) for j in range(i, self.num_genes)]
        # coloc_matrix = np.ones((self.num_genes, self.num_genes))
        # expected_coloc = np.zeros((self.num_genes, self.num_genes))
        # with mp.Pool(
        #     processes=self.threads,
        #     initializer=self._initializer_func,
        #     initargs=(self.transcript_count, self.gene_pair_weight, self.denom_per_cell,
        #               self.n_edges_per_cell, self.obs, self.min_transcript,
        #               self.high_precision, self.num_cells),
        #     maxtasksperchild=1,
        # ) as pool:
        #     for i, j, pv, exp_pv in pool.imap_unordered(ConditionalGlobalColocalization._compute_pval_parallel, tasks):
        #         coloc_matrix[i, j], expected_coloc[i, j] = pv, exp_pv
        #         coloc_matrix[j, i], expected_coloc[j, i] = pv, exp_pv
        tasks = [[i, j] for i in range(self.num_genes) for j in range(i, self.num_genes)]
        coloc_matrix = np.ones((self.num_genes, self.num_genes))
        expected_coloc = np.zeros((self.num_genes, self.num_genes))

        # CPB has many gene-pair tasks. Do not restart workers after every task.
        # maxtasksperchild=1 makes large datasets extremely slow because it repeatedly
        # destroys and recreates worker processes.
        chunksize = max(1, min(512, len(tasks) // max(self.threads * 32, 1)))
        print(f"Running global CPB on {self.threads} threads for {len(tasks)} gene pairs, chunksize={chunksize}")
        ...
        with mp.Pool(
            ...
            initargs=(
                ...
                self.gene_pair_weight,
                self.denom_per_cell,
                self.n_edges_per_cell,
                self.obs,
                self.min_transcript,
                self.high_precision,
                self.num_cells,
            ...
            for i, j, pv, exp_pv in pool.imap_unordered(
                ConditionalGlobalColocalization._compute_pval_parallel,
                tasks,
                chunksize=chunksize,
            ):
                coloc_matrix[i, j], expected_coloc[i, j] = pv, exp_pv
                coloc_matrix[j, i], expected_coloc[j, i] = pv, exp_pv
        mode = "High" if self.high_precision else "Low"
...
class Instant():
    ...
    def _calculate_ProximalPairs(self, args):
        global genes_all_cells
        cell_num, cell_id = args[0], args[1]
        pp_model = ProximalPairs(
            geneList=self.geneList,
            df_loc=cell_id,
            distance_threshold=self.distance_threshold,
            genes=genes_all_cells,
        )
        return cell_num, pp_model.p_vals, pp_model.genecount.values.reshape(len(self.geneList))
    
    def _initializer_func_pp(self, X, X_shape, genes=None):
        global position_all_cells, position_all_cells_shape, genes_all_cells
        position_all_cells = X
        position_all_cells_shape = X_shape
        genes_all_cells = genes
    ...
    def run_ProximalPairs(self, distance_threshold, min_genecount, pval_matrix_name = None, gene_count_name = None, randomize = False):
        ...
        for n, cell_id in enumerate(ids):
            ...
            if counts[n] > min_genecount:
                valid_cell_data.append([n, cell_id])
            else:
        ...
        # with mp.Pool(
        #     processes=self.threads,
        #     initializer=self._initializer_func_pp,
        #     initargs=(position_matrix, position_matrix.shape, genes),
        #     maxtasksperchild=1,
        # ) as pool:
        #     if randomize:
        #         print(f"Running randomized PP now on {self.threads} threads for, {len(valid_cell_data)} cells, {num_transcripts} transcripts")
        #         iterator = pool.imap_unordered(self._calculate_ProximalPairs_random, valid_cell_data)
        #     else:
        #         print(f"Running PP now on {self.threads} threads for, {len(valid_cell_data)} cells, {num_transcripts} transcripts")
        #         iterator = pool.imap_unordered(self._calculate_ProximalPairs, valid_cell_data)
        #     for cell_num, p_vals, genecount in iterator:
        #         self.all_pval[cell_num] = p_vals
        #         self.all_gene_count[cell_num] = genecount
        self.all_pval = np.ones((num_cells, len(self.geneList), len(self.geneList)), dtype=self.precision_mode)
        self.all_gene_count = np.zeros((num_cells, len(self.geneList)), dtype=self.precision_mode)

        # Print the dense PP tensor memory estimate. This does not change results;
        # it only helps diagnose whether the run is too large for memory.
        pp_bytes = np.prod(self.all_pval.shape) * np.dtype(self.all_pval.dtype).itemsize
        print(f"Estimated dense PP tensor memory: {pp_bytes / (1024 ** 3):.2f} GB")

        # PP is one task per valid cell. Reusing workers is much faster than
        # maxtasksperchild=1, especially for thousands of cells.
        chunksize = max(1, min(128, len(valid_cell_data) // max(self.threads * 8, 1)))
        print(f"PP multiprocessing chunksize={chunksize}")

        done = 0
        total = len(valid_cell_data)
        last_print = timeit.default_timer()

        def _store_pp_result(cell_num, p_vals, genecount):
            ...
            if done == 1 or done % 10 == 0 or done == total or now - last_print >= 60:
                elapsed = now - start
                speed = done / elapsed if elapsed > 0 else 0
                eta = (total - done) / speed if speed > 0 else float("nan")

                print(
                    f"PP progress: {done}/{total} cells "
                    f"({done / total * 100:.1f}%), "
                    f"elapsed={elapsed / 60:.1f} min, "
                    f"speed={speed:.2f} cells/s, "
                    f"ETA={eta / 60:.1f} min",
                    flush=True,
                )
                last_print = now
        ...
        if self.threads <= 1:
            print(
                f"Running PP in SERIAL mode for {len(valid_cell_data)} cells, "
                f"{num_transcripts} transcripts",
                flush=True,
            )

            self._initializer_func_pp(position_matrix, position_matrix.shape, genes)

            for item in valid_cell_data:
                if randomize:
                    cell_num, p_vals, genecount = self._calculate_ProximalPairs_random(item)
                else:
                    cell_num, p_vals, genecount = self._calculate_ProximalPairs(item)

                _store_pp_result(cell_num, p_vals, genecount)

        else:
            print(
                f"Running PP now on {self.threads} threads for, "
                f"{len(valid_cell_data)} cells, {num_transcripts} transcripts",
                flush=True,
            )
            ...
            with mp.Pool(
                ...
                if randomize:
                    iterator = pool.imap_unordered(
                        self._calculate_ProximalPairs_random,
                        valid_cell_data,
                        chunksize=chunksize,
                    )       
                else:
                    iterator = pool.imap_unordered(
                        self._calculate_ProximalPairs,
                        valid_cell_data,
                        chunksize=chunksize,
                    )
    ...
    def run_ProximalPairs3D(self, distance_threshold, min_genecount, pval_matrix_name = None, gene_count_name = None, randomize = False):
        ...
        #     for cell_num, p_vals, genecount in pool.imap_unordered(self._calculate_ProximalPairs3D, valid_cell_data):
        #         self.all_pval[cell_num] = p_vals
        #         self.all_gene_count[cell_num] = genecount
        self.all_pval = np.ones((num_cells, len(self.geneList), len(self.geneList)), dtype=self.precision_mode)
        self.all_gene_count = np.zeros((num_cells, len(self.geneList)), dtype=self.precision_mode)
        ...
        pp_bytes = np.prod(self.all_pval.shape) * np.dtype(self.all_pval.dtype).itemsize
        print(f"Estimated dense PP-3D tensor memory: {pp_bytes / (1024 ** 3):.2f} GB")

        chunksize = max(1, min(128, len(valid_cell_data) // max(self.threads * 8, 1)))
        print(f"PP-3D multiprocessing chunksize={chunksize}")
        ...
        # ) as pool:
        #     print(f"Running PP-3D now on {self.threads} threads for, {len(valid_cell_data)} cells, {num_transcripts} transcripts")
        #     for cell_num, p_vals, genecount in pool.imap_unordered(
        #         self._calculate_ProximalPairs3D,
        #         valid_cell_data,
        #         chunksize=chunksize,
        #     ):
        #         self.all_pval[cell_num] = p_vals
        ...
        with mp.Pool(
            ...
            print(f"Running PP-3D now on {self.threads} threads for, {len(valid_cell_data)} cells, {num_transcripts} transcripts", flush=True)
            ...
            for cell_num, p_vals, genecount in pool.imap_unordered(
                ...
                if done == 1 or done % 50 == 0 or done == total or now - last_print >= 60:
                    elapsed = now - start
                    speed = done / elapsed if elapsed > 0 else 0
                    eta = (total - done) / speed if speed > 0 else float("nan")

                    print(
                        f"PP-3D progress: {done}/{total} cells "
                        f"({done / total * 100:.1f}%), "
                        f"elapsed={elapsed / 60:.1f} min, "
                        f"speed={speed:.2f} cells/s, "
                        f"ETA={eta / 60:.1f} min",
    ...
    def annotate_ProximalPairs(self, distance_threshold, distance_thresold_nucleus = 2.5, distance_threshold_cyto_nuclear = 2.5, distance_threshold_cyto_peri = 4):
        ...
        for n, cell_id in enumerate(cell_ids):
            ...
            # 只把空间坐标送进 KDTree
            coord_cols = [c for c in ["absX", "absY", "absZ"] if c in self.curr_cell_df.columns]
            # 2D 数据
            if "absZ" not in coord_cols:
                coord_cols = ["absX", "absY"]

            coords = self.curr_cell_df[coord_cols].to_numpy(dtype=np.float64)
