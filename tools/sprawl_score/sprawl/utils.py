...
def _iter_vars(cells, processes=2):
    ...
    If processes <= 1: run serially (NO multiprocessing / NO Manager).
    Else: run multiprocessing.
    """
    if processes is None:
        processes = 1

    if processes <= 1:
        # serial path: avoid mp.Manager / mp.Pool hangs
        var_mem = {}
        for cell in cells:
            yield _cell_var(cell, var_mem=var_mem)
        return
